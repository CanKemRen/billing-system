package de.personalmarkt.commands.format;
...
public class MapFormat extends Format {
	 ...
	 * parses enclosed string and replaces it with argument's <code>get()</code> value.
	 * 
	 * @param pattern
	 *            String to be parsed.
	 * @param arguments
	 ...
	 * @param rmark
	 *            Right mark of to-be-skipped block or null if does not exist (// comment). private void process(String pattern, String lmark, String rmark) {
	 *            int idx = 0; while (true) { int ridx = -1, lidx = pattern.indexOf(lmark,idx); if (lidx >= 0) { if (rmark != null) { ridx =
	 ...
	 */
	/**
	 * Returns the value for given key. Subclass may define its own beahvior of this method. For example, if key is not defined, subclass can return <not
	 * defined> string.
	 *
	 * @param key
	 *            Key.
	 ...
	 *                if number of arguments exceeds BUFSIZE or parser found unmatched brackets (this exception should be switched off using
	 *                setExactMatch(false)).
	 */
	...
	/**
	 * Test whether formatter will throw exception if object for key was not found. If given map does not contain object for key specified, it could throw an
	 * exception. Returns true if throws. If not, key is left unchanged.
	 */
	...
	/**
	 * Specify whether formatter will throw exception if object for key was not found. If given map does not contain object for key specified, it could throw an
	 * exception. If does not throw, key is left unchanged.
	 * 
	 * @param flag
	...
	/**
	 * Test whether both brackets are required in the expression. If not, use setExactMatch(false) and formatter will ignore missing right bracket. Advanced
	 * feature.
	 */
	...
	/**
	 * Specify whether both brackets are required in the expression. If not, use setExactMatch(false) and formatter will ignore missing right bracket. Advanced
	 * feature.
	 * 
	 * @param flag
	 *            If true, formatter will ignore missing right bracket (default = false)
	 */
	...
	/**
	 * Sets argument map This map should contain key-value pairs with key values used in formatted string expression. If value for key was not found, formatter
	 * leave key unchanged (except if you've set setThrowExceptionIfKeyWasNotFound(true), then it fires IllegalArgumentException.
	 *
	 * @param map
	 *            the argument map
	 */
